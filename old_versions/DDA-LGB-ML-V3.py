...
@optunity.cross_validated(x=X_train, y=y_train, num_folds=5, num_iter=2)
def objective(params, x_train, y_train, x_test, y_test):
    train_data = lgb.Dataset(x_train, label=y_train)
    test_data = lgb.Dataset(x_test, label=y_test, reference=train_data)

    bst = lgb.train(params, train_data, 100000, valid_sets=[test_data], verbose_eval=False)
...
optimal_params, _, _ = optunity.minimize(objective, num_evals=50, solver_name='particle swarm', **search_space)
optimal_params
